...
class TestConsensusEngine:
    ...
    def test_submit_vote(self):
        engine = ConsensusEngine()
        # Adjust according to actual method signature
        engine.submit_vote(
            finding_id="f1",
            verdict="valid",      # or status / decision depending on real impl
            confidence=0.9,
        )
    ...
    def test_consensus_reached_with_quorum(self):
        ...
        assert result.get("consensus") in ["valid", "accepted", True]
    ...
    def test_confidence_fusion(self):
        ...
        engine.submit_vote("f4", verdict="valid", confidence=0.85)
