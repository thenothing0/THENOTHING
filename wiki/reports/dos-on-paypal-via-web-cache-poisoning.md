---
type: report
tags:
- report
- auto
target: '[[paypal]]'
severity: medium
created: '2026-06-26'
updated: '2026-06-26'
source: https://hackerone.com/reports/622122
vuln_class: unknown
asset_type: web
learning_score: 2
learning_score_rationale: base 2 (low-signal class 'unknown')
unresolved_references: []
---

# DoS on PayPal via web cache poisoning

> Reusable lesson distilled from a disclosed report — see the intel page [[dos-on-paypal-via-web-cache-poisoning-intel]].

## Distilled intelligence
- **Root cause:** unknown  
  <sub>provenance: not found</sub>
- **Trust-boundary failure:** unknown  
  <sub>provenance: not found</sub>
- **Exploitation sequence:**   
  <sub>provenance: not found</sub>
- **Escalation / impact:** unknown  
  <sub>provenance: not found</sub>
- **Impact:** unknown  
  <sub>provenance: not found</sub>
- **Severity reasoning:** unknown  
  <sub>provenance: no explicit severity statement</sub>
- **Attacker assumptions:** unknown  
  <sub>provenance: not found</sub>

## Why the learning_score
- **2/10** — base 2 (low-signal class 'unknown')
- signals: none

## Related
- Intel: [[dos-on-paypal-via-web-cache-poisoning-intel]] · Target: [[paypal]]
