...
It registers **239 tools** spanning a curl-first general shell → recon → scanning → gated PoC exploitation →
...
state machine → dynamic external-MCP discovery → multi-client engagements + RBAC + report export (SARIF/MD/JSON)
→ an offline-first Knowledge OS (Phases A–U). They are **deferred / search-loaded** — invoke any tool by name; you do NOT
