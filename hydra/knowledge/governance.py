...
class GovernanceIntelligence(_GovernanceBase):
    ...
    def adversary_intelligence(self) -> Dict:
        """Phase-T adversary/ATT&CK-intelligence health (adversary_health_score / technique coverage /
        tactic coverage / mean covered effectiveness). Read-only; lazy import keeps governance robust
        if the layer is absent. Never alters confidence/promotion or the wiki; NON-executing."""
        try:
            from hydra.adversary_intel.intelligence import AdversaryIntelligence
            return AdversaryIntelligence().attack_health()
        except Exception:
