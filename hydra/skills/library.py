...
def _build_core_library() -> SkillRegistry:
    """Build core skill library (base skills only)."""
    reg = SkillRegistry()
...
def build_full_library() -> SkillRegistry:
    """Build and return the complete skill library with all extensions."""
    reg = _build_core_library()

    # Register all extended skill packs
    from hydra.skills.extended_web import register_all_extended
    from hydra.skills.extended_advanced import register_all_extended_v2
    register_all_extended(reg)
    register_all_extended_v2(reg)

    try:
        from hydra.skills.evolution_store import load_registry_metrics
        from hydra.skills.yaml_loader import register_yaml_skills

        register_yaml_skills(reg)
        load_registry_metrics(reg)
    except Exception as exc:  # noqa: BLE001 — library must stay usable without YAML disk
        logger.warning("YAML skills / evolution load skipped: %s", exc)
