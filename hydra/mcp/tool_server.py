...
TOOL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "sqlmap": {
        ...
        "build_cmd": lambda params: [
            "sqlmap", "-u", params["target"], "--batch", "--level=1", "--risk=1",
        ],
    ...
    "dalfox": {
        ...
        "build_cmd": lambda params: [
            "dalfox", "url", params["target"], "--silence",
        ],
        "description": "XSS parameter analysis and scanner",
    },
    "gxss": {
        "binary": "gxss",
        "install": {"go": "go install github.com/KathanP19/Gxss@latest"},
        "build_cmd": lambda params: ["gxss"],
        "stdin_mode": True,
        "description": "Check reflected parameters for XSS",
    },
    "dnsx": {
        "binary": "dnsx",
        "install": {"go": "go install -v github.com/projectdiscovery/dnsx/cmd/dnsx@latest"},
        "build_cmd": lambda params: [
            "dnsx", "-silent", "-a", "-resp", "-d", params["target"],
        ],
        "description": "Fast DNS resolution and enumeration",
    },
    "hakrawler": {
        "binary": "hakrawler",
        "install": {"go": "go install github.com/hakluke/hakrawler@latest"},
        "build_cmd": lambda params: [
            "hakrawler", "-url", params["target"], "-depth", "2", "-plain",
        ],
...
TOOL_ALIASES = {
    ...
    "dir_brute": ["dirsearch"],
    "sqli_scan": ["sqlmap"],
    "xss_scan": ["dalfox", "gxss"],
    "dns_enum": ["dnsx"],
    "web_crawl": ["katana", "hakrawler"],
}
